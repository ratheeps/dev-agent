...
class PRCreationHandler:
    ...
    async def create_pr(
        ...
        *,
        ...
        """Create a PR for a single repo, link to Jira, and notify Slack.
        ...
        await jira_client.add_comment(
            key=task.jira_key,
            body=f"PR created by Mason: {pr.url}",
        )
    ...
    async def create_prs(
        ...
        *,
        ...
        if pr_urls:
            ...
            await jira_client.add_comment(
                key=task.jira_key,
                body=f"Mason created {len(pr_urls)} PR(s):\n{pr_list}",
            )
...
---
*Generated by Mason Agent System*
"""
