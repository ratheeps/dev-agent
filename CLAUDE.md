...
## Tech Stack
- **Language:** Python 3.10+ (3.12 preferred)
- **Package manager:** uv
...
dev-ai/
  ...
  src/
    integrations/         # MCP client wrappers
      atlassian/          # Jira + Confluence clients
      figma/              # Design ingestion client
      github/             # Repo/PR management client
      teams/              # Notifications + approval bot
      mcp_manager.py      # Central MCP connection manager
    agents/               # Orchestrator + worker agents
      orchestrator.py     # Opus: plan, delegate, review
      worker.py           # Sonnet: implement, test, commit
      base.py             # Shared agent base class
      registry.py         # Agent discovery/lifecycle
      communication.py    # Inter-agent message protocol
    schemas/              # Pydantic models (task, plan, message)
    prompts/              # System prompts (markdown)
    memory/               # Bedrock AgentCore Memory client
    workflows/            # Jira-to-PR state machine
    handlers/             # Webhook + CLI entry points
...
## Conventions
- **Async by default:** All MCP calls and agent operations are async (httpx, asyncio).
- **Pydantic for all data:** Use Pydantic v2 models for tasks, plans, messages — no raw dicts crossing module boundaries.
- **Type hints everywhere:** mypy strict mode is enforced.
- **Config over code:** Timeouts, limits, model choices live in `config/*.yaml`, not hardcoded.
